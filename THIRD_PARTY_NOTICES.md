# Third-party notices

This repository contains original workflow instructions, templates, and helper scripts. It does not redistribute the full text or PDF files of third-party documentation.

The project links to official documentation for these services:

- Volcengine Ark / Doubao Seedance
- Google Gemini API
- OpenAI / Codex image generation
- FFmpeg

Product names and trademarks belong to their respective owners. Content reached through external links remains governed by the terms and copyright notices of its publisher. The MIT License in this repository applies only to the original material stored in this repository.
